...
A: Three technical reasons. (1) **Async-first**: every external call (Jira, GitHub, LLM) is
`await`-ed. Flask's sync routes would block per-request; Django async support requires ASGI
configuration overhead. FastAPI is async-native. (2) **Pydantic integration**: request/response
schemas are Pydantic models. Validation, serialisation, and OpenAPI documentation are automatic. No
additional validation library needed. (3) **Developer experience**: auto-generated Swagger UI at
`/docs` means the API is self-documenting for the QA engineers who call it.

**Q: Why five separate LLM prompts instead of one mega-prompt returning all five outputs?**
A: Five reasons. (1) **Failure isolation**: if BDD generation fails (malformed JSON, context window
exceeded), test cases are still available. Monolithic failure means total loss. (2) **Prompt
optimisation**: each prompt has a distinct system role. The quality validator acts as a QA principal;
the alignment checker acts as a code reviewer; the test case generator acts as a test architect.
Mixed roles degrade output quality. (3) **Output shape**: five different JSON schemas in one
response requires the LLM to maintain structure coherence across a much larger generation. Separate
calls with separate schemas are more reliable. (4) **Independent tuning**: each step can be improved
independently without affecting others. (5) **Inspectability**: intermediate outputs are saved to
disk and visible in the UI, allowing partial results to be used even if later steps fail.

**Q: How does `call_structured()` handle provider differences in JSON mode support?**
A: All currently supported providers (GitHub Models gpt-4o, OpenAI, Azure OpenAI, Anthropic via
OpenAI-compatible proxy, Groq) support `response_format={"type": "json_object"}` when the system
prompt explicitly instructs JSON-only output. For providers that don't support JSON mode (older
Ollama models, some Groq models), the `supports_json_mode: false` flag in the provider config
disables the response_format parameter. The multi-tier extraction fallback (direct parse → markdown
strip → regex outermost-object) handles the remaining inconsistencies.

**Q: What's the memory footprint for a typical request?**
A: A full workflow run for one ticket holds approximately 50–200KB in memory: ticket dict (~5KB),
commits list (~15KB), 5 LLM responses (~10–30KB each), generated feature file (~5KB), step defs
(~10KB), HTML report (~50KB). All intermediate state is in the `QAResponse` object, discarded after
the request completes. There is no server-side session state. The `outputs/` files persist on disk
for download.
...
A: `temperature=0.3`. Closer to 0 (deterministic) than 1 (creative). For QA outputs — test IDs,
step sequences, JSON schemas — consistency and parsability matter more than novelty. At 0.0, some
providers produce slightly less varied test case coverage. At 0.3, outputs vary slightly between
runs (which reflects legitimate scenario diversity) while remaining well-structured. BDD generation
uses the same setting because Gherkin has strict syntactic requirements.

**Q: How do you prevent the LLM from ignoring team context?**
A: The `additional_context` is appended to the user section of every prompt (not the system
section). User-section content has higher instruction-following weight in most RLHF-tuned models
than late-appended system context. The context block is prefixed with `--- Team Context ---` and
`--- Custom Instructions ---` headings, which helps the model identify it as authoritative input
rather than background information.

**Q: What happens when the LLM hallucinates a step definition that doesn't match any Gherkin in the feature file?**
A: This is caught at review time. The UI renders both the feature file and the step definitions
side by side. A QA engineer verifies the step text alignment before pushing. In Phase 2, automated
step text extraction and matching will surface misaligned steps in the UI before push, so the
engineer is explicitly flagged rather than having to spot the mismatch manually.

**Q: How do you handle the case where a ticket has no description and no acceptance criteria — just a title?**
A: The quality validator scores it critically (typically 20–40/100), generates a blocking issue:
`severity: "critical", message: "No description or acceptance criteria..."`. The workflow continues
with all available context. The LLM generates test cases based on the title alone (which it can
often infer reasonable scenarios for), but the quality score prominently signals to the QA engineer
that the ticket needs enrichment before review.
...
A: All secrets (Jira URL + email + API token, GitHub PAT, LLM API keys) are loaded exclusively from
environment variables via `pydantic.BaseSettings`. They are never written to source code, logged,
returned in API responses, or stored in `outputs/`. The `/api/v1/run-qa` endpoint does not echo
credentials back in any form. The Pydantic settings class marks sensitive fields with `repr=False`
to prevent accidental logging during debug.

**Q: Is the Jira API token exposed in the generated HTML reports?**
A: No. `report_service.generate_html_report()` receives the `QAResponse` dict which contains only
the processed results (scores, issues, test cases) — not the raw Jira ticket payload. The ticket
title and description are included in the report but no authentication context is carried through.

**Q: What happens if an attacker submits a malicious Jira ticket ID like `../../etc/passwd`?**
A: `jira_service.get_ticket()` constructs the Jira URL by concatenating the base URL with the ticket
ID. The Jira REST API rejects any ID that doesn't match the `PROJECT-NNN` format with a 400 or 404.
The app does not use the ticket ID to construct local file paths — artefact paths use
`sanitize_filename(jira_id)` which strips all characters except alphanumerics and hyphens.

**Q: How is the "Upload to Jira" content sanitised?**
A: The `JiraUploadRequest` body is validated by Pydantic (string fields with max length). Before
the Jira comment POST, the `summary` and `issues` fields are escaped for Atlassian Document Format
(ADF) — special ADF control characters are stripped. The content is not rendered as HTML in Jira
(ADF, not HTML), so XSS via Jira's editor is not applicable. Script injection into ADF text nodes
...
A: 80% line coverage for `app/services/` (measured via `pytest-cov`). `ai_service._mock_response()`
is fully covered by unit tests since it is the deterministic offline path. LLM integration paths
are covered by integration tests that run only when `RUN_INTEGRATION_TESTS=true`.

**Q: How do you handle Jira API downtime?**
A: `jira_service.get_ticket()` has a configurable timeout (default 30s). On timeout or non-2xx
response, it raises `JiraServiceError` with the HTTP status and response body. `workflow_service`
catches this at Step 1 and returns a `QAResponse` with `error: "Jira fetch failed: <message>"`.
The UI renders the error message in the report panel rather than a crash. GitHub and LLM failures
follow the same pattern — each step has a specific error type that maps to an `error` field on the
response.

---

### Career and Growth Context

**Q: What was the most instructive mistake during this build?**
A: Generating plain pytest functions for the automation repo before auditing the repo's actual
framework. The first implementation was built assuming "pytest + Playwright = standard pytest
functions". When the user updated the repo to pytest-bdd, the generated output was structurally
incompatible — wrong imports, wrong decorators, wrong file location conventions. Fixing this
required: rewriting the prompt (40+ lines), updating the mock response, updating the file header
writer, updating three log messages, and creating this Q&A entry. The cost of the mistake was
about 2 hours of rework. The lesson cost zero lines — always read the target repo's existing
files before writing a code generator for it.

**Q: If you were advising another engineer building a similar AI workflow, what would you tell them?**
A: Five things. (1) Separate prompts for separate concerns — multi-output monolithic prompts fail
unpredictably. (2) Always validate LLM output against a Pydantic schema before using it — never
trust raw completions. (3) Keep the human review gate mandatory — auto-posting AI content to
authoritative systems is a trust and accuracy problem, not just a preference. (4) Build the mock
mode first — it makes unit tests fast and deterministic, which accelerates iteration. (5) Store
team context in editable flat files, not code — business rules change without warning and your
users should be able to update them without a deployment.

**Q: What would you now add to this platform if you could spend another two weeks on it?**
A: In priority order: (1) PostgreSQL persistence for quality trend dashboards — the most requested
feature from QA leads. (2) GitHub Actions trigger on PR open — shifts QA left to the code review
moment. (3) Duplicate step detection before push — catches the most common push-time rejection.
(4) Step execution loop — trigger the test run, pull results, render pass/fail on the report. (5)
MCP server — makes the platform composable for AI agent workflows in tools like Copilot and Claude
Desktop.
